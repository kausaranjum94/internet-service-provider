 ...
 */
function internet_service_provider_pro_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','internet-service-provider-pro' ),
...
add_action( 'customize_register', 'internet_service_provider_pro_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function internet_service_provider_pro_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = internet_service_provider_pro_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = internet_service_provider_pro_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'internet_service_provider_pro_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function internet_service_provider_pro_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function internet_service_provider_pro_customize_preview_js() {
...
add_action( 'customize_preview_init', 'internet_service_provider_pro_customize_preview_js' );

/**
 * JS handlers for Customizer Controls
 */
function internet_service_provider_pro_customize_controls_js() {
...
add_action( 'customize_controls_enqueue_scripts', 'internet_service_provider_pro_customize_controls_js' );

/**
 * Testing function to remove logo_size theme mod
 */
function internet_service_provider_pro_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'internet_service_provider_pro_remove_theme_mod' );